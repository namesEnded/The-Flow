...
saveButton.addEventListener('click', (url) => {
  const canvasElement = imageContainer.querySelector('canvas');
  if (canvasElement) {
      const format = prompt('Введите формат файла (png, jpeg, webp):', 'png');
      if (format === 'png' || format === 'jpeg' || format === 'webp') {
            ...
            async blob => {
                try {
                    ...
                    if (jsonResponse.uploaded) {
                        ...
                        let value = saveButton.getAttribute("data-state");
                        ...
                        // Формируем новый URL с новым путем (страницей)
                        let newUrl = currentUrl.replace(/\/current-page$/, jsonResponse.url);

                        // Выполняем редирект на новую страницу
