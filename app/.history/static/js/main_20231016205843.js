...
async function loadImageFromUrl(url) {
  try {
    ...
    imageName.style.display = 'none';
    imageButton.style.display = 'none';
    throw e; // перебрасываем ошибку, чтобы она могла быть обработана вне функции
  }
}
...
async function deleteImageByUrl(url) {
  try {
    ...
    throw error; // перебрасываем ошибку, чтобы она могла быть обработана вне функции
  }
}
...
Thinking ... 
Для того чтобы функции можно было вызывать последовательно друг за другом, их нужно преобразовать в асинхронные функции, возвращающие промисы. Это позволит использовать их с помощью async/await для более удобного управления потоком выполнения. Вот как это можно сделать:
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = function() {
          ...
          resolve(); // вызываем resolve, когда изображение успешно загружено и обработано
        };
        img.onerror = function() {
          reject('Ошибка при загрузке изображения');
        };
...
async function processImage(url) {
  try {
    ...
    // ... здесь может быть код, который должен выполниться после загрузки изображения
    await deleteImageByUrl(url);
    // ... здесь может быть код, который должен выполниться после удаления изображения
  } catch (error) {
